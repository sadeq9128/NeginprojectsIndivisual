...
const ForumCreate = (props) => {
  ...
  const getMachineInfo=(machineCode,vehicle,name)=>{
    message.destroy(key);
    if((machineCode+"").length<5) return;
  ...
  const getPersonInfo=(perosnCode,personnel,name)=>{
    message.destroy(key);
    if((perosnCode+"").length<10) return;
  ...
  const finishForm=(values)=>{
    ...
    console.log(values);
    if(values!==null){
      axios.put( "/admin/incident",values )
        .then( response => {
        ...
        .catch( error => {
            message.error("خطا در بروزرسانی حادثه");
        } );
    }else{
  ...
  if(props.data!==null){
    let value=props.data.incident;
    value.date=value.date.split(" ")[0].replace("-","/").replace("-","/");
    value.personnel=props.data.persons;
    for(let p of value.personnel){
      p.national_id=p.national_number;
    }
    try{
      value.reason=value.reason.split(",");
    }catch(e){}
    try{
      value.type=value.type.split(",");
    }catch(e){}
    value.vehicles=props.data.vehicle;
    value.persons=props.data.persons;
                        ...
                        return (machineCode!==undefined &&
                          machineCode['machine_code']) ? (
                            <Row>
                              ...
                              >
                                <Input disabled={true} placeholder="سیستم" />
                              </Form.Item>
                              </Col>
                              <Col xs={2} sm={8} md={10} lg={10} xl={10}>
                              <Form.Item
